...
boost::optional<std::pair<double, double>> Utils::SolveFunction(const double a, const double b, const double c) {
...
double Utils::AfterRotate(double init_yaw, double target_yaw, double t, const double yaw_rate_limit) {
  double res_yaw = init_yaw;
  if (init_yaw >= 0 && target_yaw >= init_yaw) {
    res_yaw = (((target_yaw - init_yaw) / yaw_rate_limit <= t) ? target_yaw : (init_yaw + t * yaw_rate_limit));
